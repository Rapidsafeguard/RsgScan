...
class Logic():
    ...
    def createTemporaryFiles(self):
        try:
            ...
            log.info(self.cwd)
            tf = tempfile.NamedTemporaryFile(suffix=".rsgscan",prefix="rsgscan-", delete=False, dir="./tmp/")         # to store the database file
            self.outputfolder = tempfile.mkdtemp(suffix="-tool-output",prefix="rsgscan-", dir="./tmp/")            # to store tool output of finished processes
            self.runningfolder = tempfile.mkdtemp(suffix="-running",prefix="rsgscan-", dir="./tmp/")               # to store tool output of running processes
            ...
            self.usernamesWordlist = Wordlist(self.outputfolder + '/rsgscan-usernames.txt')          # to store found usernames
            self.passwordsWordlist = Wordlist(self.outputfolder + '/rsgscan-passwords.txt')          # to store found passwords
