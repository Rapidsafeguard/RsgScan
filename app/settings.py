...
class AppSettings():
    def __init__(self):
        ...
        if not os.path.exists('./rsgscan.conf'):
            log.info('Legion config is missing. Please reclone.')
            os.exit(1)
        else:
            log.info('Loading settings file..')
            self.actions = QtCore.QSettings('./rsgscan.conf', QtCore.QSettings.NativeFormat)
    ...
    def backupAndSave(self, newSettings, saveBackup = True):
        ...
        if saveBackup:
            ...
            os.rename('./rsgscan.conf', './backup/' + getTimestamp() + '-rsgscan.conf')
        else:
